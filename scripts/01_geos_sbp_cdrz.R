...
rural_urban = read.csv(here("data", "data-raw", "rural_urban_continuum_code_2023.csv")) %>% 
  ...
  select(county_state_code, urban_designation, description)
...
sbp_portfolio <- cdrzs_tracts_places_clean %>% 
  ...
  select(-tribal_name, -objectid,-state_fips, 
         ...
         -nicknames_geos, -host_entity_geos, 
...
#Adding county_population to the data set 
all_cdrzs_full_data <- merge(
  walmart_cdrzs_nri,
  spb_geos_countY_population,
  by.x = c("county_state_code", "state"),
  by.y = c("county_state_code", "state"),
  all.x = TRUE
)
...
unique(partner_cdrz_all_data$description)
